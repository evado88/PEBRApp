...
class R21AddEventScreen extends StatelessWidget {
  final Patient _patient;
  final String _title;
  final R21ScreenAnalytic _analytic =
      new R21ScreenAnalytic(type: R21ScreenType.AddEvent());
  ...
  Widget build(BuildContext context) {
    ...
    return PopupScreen(
        ...
        child: R21AddEventForm(_patient, _analytic));
  }
}

class R21AddEventForm extends StatefulWidget {
  final Patient _patient;
  final R21ScreenAnalytic _analytic;

  R21AddEventForm(this._patient, this._analytic);

  @override
  createState() => _R21AddEventFormState(_patient, _analytic);
}

class _R21AddEventFormState extends State<R21AddEventForm> {
  // fields
  ...
  // constructor
  _R21AddEventFormState(this._patient, this._analytic) {
    _event = R21Event(patientART: this._patient.personalStudyNumber);
  }
  ...
  Widget _noOccurReasonQuestion() {
    return _makeQuestion(
      'Reason why event did not occur',
  ...
  _onSubmitForm(BuildContext context) async {
    if (_formKey.currentState.validate() &&
      ...
      await DatabaseProvider().insertEvent(_event);
      _patient.events.add(_event);
